...
struct Spotify: View {
      ...
      Image("Artwork")
        ...
        .overlay {
          ZStack(alignment: .bottom) {
            Rectangle()
              .fill(.linearGradient(colors: gradientColors(progress: progress), startPoint: .top, endPoint: .bottom))
  ...
  func gradientColors(progress: Double) -> [Color] {
    return [
...
struct Spotify_Previews: PreviewProvider {
  static var previews: some View {
    GeometryReader{
      ...
      let size = $0.size
      Spotify(safeArea: safeArea, size: size)
